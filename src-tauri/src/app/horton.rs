...
#[command]
pub fn get_horton_model_cmd() -> serde_json::Value {
  let path = utils::app_root().join("horton.model.cmd.json");
  ...
  if let Some(v) = res {
    let data = parse_prompt(v)
      ...
      .map(move |i| ModelRecord {
        ...
        tags: vec!["horton-prompts".to_string()],
        enable: true,
      })
    ...
    let model = utils::app_root().join("horton.model.json");
    let model_cmd = utils::app_root().join("horton.model.cmd.json");
    let horton_prompts = utils::app_root()
      .join("horton_model")
      .join("horton_prompts.json");
    ...
    if !utils::exists(&model) {
      fs::write(
        ...
        serde_json::json!({
          "name": "Horton Model",
          "link": "https://github.com/clean-code-studio/horton-desktop"
        })
        .to_string(),
      )
      .unwrap();
    ...
    kv.insert(
      ...
      serde_json::json!({ "id": "horton_prompts", "last_updated": time }),
    );
    ...
    api::dialog::message(
      ...
      "Sync Prompts",
      "Horton Prompts data has been synchronized!",
    );
...
pub async fn sync_user_prompts(url: String, data_type: String) -> Option<Vec<ModelRecord>> {
  ...
  if let Some(v) = res {
    let data;
    if data_type == "csv" {
      info!("horton_http_csv_parse");
      data = parse_prompt(v);
    } else if data_type == "json" {
      info!("horton_http_json_parse");
      data = serde_json::from_str(&v).unwrap_or_else(|err| {
        error!("horton_http_json_parse: {}", err);
        vec![]
      });
    } else {
      error!("horton_http_unknown_type");
      data = vec![];
    }
