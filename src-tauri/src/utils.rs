...
pub fn app_root() -> PathBuf {
  tauri::api::path::home_dir().unwrap().join(".horton")
}
...
pub fn create_horton_prompts() {
  let sync_file = app_root().join("cache_model").join("horton_prompts.json");
  if !exists(&sync_file) {
...
pub fn script_path() -> PathBuf {
  ...
  if !exists(&script_file) {
    ...
    fs::write(
      &script_file,
      format!(
        "// *** Horton User Script ***\n// @github: https://github.com/clean-code-studio/horton-desktop \n// @path: {}\n\nconsole.log('🤩 Hello Horton Desktop!!!');",
...
pub fn clear_conf(app: &tauri::AppHandle) {
  ...
  let msg = format!(
    "Path: {}\n
    Are you sure you want to clear all Horton Desktop configurations? Performing this operation data can not be restored, please back up in advance.\n
...
pub async fn get_data(
  ...
  if is_ok {
    ...
    error!("horton_http: {}", body);
    if let Some(v) = app {
      tauri::api::dialog::message(v.get_window("core").as_ref(), "Horton HTTP", body);
    }
    Ok(None)
  }
}
      ...
      if update_resp.is_update_available() {
        ...
        if v {
          tauri::api::dialog::message(
            ...
            "Horton Desktop",
            "Your Horton Desktop is up to date",
          );
        }
      }
    }
